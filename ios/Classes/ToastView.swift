...
class ToastView: UIView {
    ...
    /// 显示纯文本 Toast
    static func showToast(message: String, position: Position = .bottom) {
        ...
        case .top:
            yPos = 50
        case .center:
            yPos = (window.bounds.height - toastHeight) / 2
        case .bottom:
            yPos = window.bounds.height - toastHeight - 50
        }

        toastView.frame = CGRect(x: (window.bounds.width - toastWidth) / 2, y: yPos, width: toastWidth, height: toastHeight)
    ...
    /// 显示带图标的 Toast
    static func showIconToast(message: String, type: ToastType, position: Position = .center) {
            guard let window = UIApplication.shared.keyWindow else { return }
            ...
            let toastHeight = labelSize.height + 60 + 24
    ...
    /// 显示带loading  Toast
        static func showLoadingToast(message: String, position: Position = .center) {
            guard let window = UIApplication.shared.keyWindow else { return }
